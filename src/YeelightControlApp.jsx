...
const DEFAULT_BASE = ""; // same-origin; or set to "http://pi:5005" if different host
...
<div className="flex items-center gap-2">
<Button variant="ghost" onClick={() => exec(() => onToggle(target))} disabled={busy}>
<Power className="h-4 w-4 mr-1" /> Toggle
</Button>
<Button onClick={() => exec(() => onPower(target, true))} disabled={busy}>On</Button>
<Button variant="ghost" onClick={() => exec(() => onPower(target, false))} disabled={busy}>Off</Button>
</div>
</div>
